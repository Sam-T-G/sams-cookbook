...
1. **Swift client for the Claude Messages API (resolved by the spike).** There is no official Anthropic Swift SDK. The spike verified `SwiftAnthropic` 2.2.2 (2026-04-18) against source: `Model.other("claude-opus-4-8")`, streaming, and tool use work, but it only emits the deprecated `thinking: {type: "enabled", budget_tokens: N}` shape, which `claude-opus-4-8` rejects with a 400, and it has no effort control and no structured outputs. Decision: a raw-`URLSession` `/v1/messages` client is the **primary** client for cloud-Claude recipes, because the cookbook standardizes on adaptive thinking, the effort control, and structured outputs (none of which `SwiftAnthropic` can send), and because it matches the user's own Citrus Squad hand-rolled `ClaudeClient` idiom. `SwiftAnthropic` is documented as an option for basic chat, streaming, and tool use that does not need the newer controls. This is built: `recipes/06-ai-features/cloud-claude-chat` ships the typed client with golden-vector tests over the request shape and response decoding.
